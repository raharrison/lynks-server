...
open class DefaultLinkProcessor(
    ...
    override suspend fun process(resourceSet: EnumSet<ResourceType>): Map<ResourceType, GeneratedResource> {
        ...
        if (resourceSet.contains(ResourceType.PREVIEW) || resourceSet.contains(ResourceType.THUMBNAIL)) {
            val preview = generatePreview(linkContent)
            addResource(generatedResources, ResourceType.PREVIEW, preview)
            addResource(generatedResources, ResourceType.THUMBNAIL, generateThumbnail(preview))
        }
        if (resourceSet.contains(ResourceType.READABLE)) {
            linkContent.extractedContent?.let {
                addResource(generatedResources, ResourceType.READABLE, GeneratedDocResource(it, HTML))
            }
        }
        if (resourceSet.contains(ResourceType.SCREENSHOT)) {
            addResource(generatedResources, ResourceType.SCREENSHOT, generateScreenshot())
        }
        if (resourceSet.contains(ResourceType.PAGE)) {
            addResource(generatedResources, ResourceType.PAGE, GeneratedDocResource(html, HTML))
        }
        if (resourceSet.contains(ResourceType.DOCUMENT)) {
            addResource(generatedResources, ResourceType.DOCUMENT, printPage())
        }
