...
fun Route.file(fileService: FileService) {
    ...
    route("/file") {
        ...
        get("/{id}") {
            val file = fileService.get(EntryId(call.parameters["id"] ?: throw InvalidModelException("Missing id")))
        ...
        delete("/{id}") {
            val removed = fileService.delete(EntryId(call.parameters["id"] ?: throw InvalidModelException("Missing id")))
