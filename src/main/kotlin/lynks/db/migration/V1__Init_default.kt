...
class V1__Init_default : BaseJavaMigration() {
    ...
    private fun createPostgresEntrySearch(conn: Connection) {
        conn.createStatement().use {
            ...
            // text_pattern_ops lets the title prefix LIKE in suggest use it under a non-C collation
            it.execute("CREATE INDEX entries_user_title_lower_idx ON ${Entries.tableName} (${Entries.userId.name}, lower(${Entries.title.name}) text_pattern_ops);")
        }
    }
