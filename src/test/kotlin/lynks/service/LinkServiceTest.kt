...
class LinkServiceTest : DatabaseTest() {
    ...
    fun testCreateBasicLink() {
        val link = linkService.add(TEST_USER, newLink("n1", "google.com/page"))
    ...
    fun testCreateLinkWithTags() {
        val link = linkService.add(TEST_USER, newLink("n1", "google.com", listOf("t1", "t2")))
        assertThat(link.type).isEqualTo(EntryType.LINK)
        assertThat(link.title).isEqualTo("n1")
    ...
    fun testCreateLinkWithCollections() {
        val link = linkService.add(TEST_USER, newLink("n1", "google.com", emptyList(), listOf("c1", "c2")))
        assertThat(link.type).isEqualTo(EntryType.LINK)
        assertThat(link.title).isEqualTo("n1")
        assertThat(link.url).isEqualTo("https://google.com")
    ...
    fun testCreateWithNoProcessFlag() {
        ...
        linkService = LinkService(groupSetService, entryAuditService, resourceManager, workerRegistry)
        val link =
            linkService.add(TEST_USER, newLink(EntryId("n1"), "google.com", "url", listOf("t1", "t2"), listOf("c1"), false))

        verify(exactly = 1) { workerRegistry.acceptLinkWork(any()) }
        verify(exactly = 0) { workerRegistry.acceptDiscussionWork(TEST_USER, link.id) }
        verify(exactly = 1) { entryAuditService.acceptAuditEvent(link.id, any(), any()) }
    }

    @Test
    fun testCreateLinkWithInvalidTag() {
        assertThrows<InvalidModelException> { linkService.add(TEST_USER, newLink("n1", "google.com", listOf("t1", "invalid"))) }
    }
    ...
    @Test
    fun testGetLinkById() {
        linkService.add(TEST_USER, newLink("n1", "google.com", listOf("t1", "t2"), listOf("c1")))
        val link2 = linkService.add(TEST_USER, newLink("n2", "google.com", listOf("t2"), listOf("c2")))
        val retrieved = linkService.get(TEST_USER, link2.id)
    ...
    fun testGetLinkDoesntExist() {
        assertThat(linkService.get(TEST_USER, EntryId("invalid"))).isNull()
    }
    ...
    fun testGetLinksPage() {
        linkService.add(TEST_USER, newLink("n1", "google.com", listOf("t1", "t2"), listOf("c1")))
        Thread.sleep(10)
        linkService.add(TEST_USER, newLink("n2", "amazon.com", listOf("t1", "t2"), listOf("c1")))
        Thread.sleep(10)
        linkService.add(TEST_USER, newLink("n3", "netflix.com", listOf("t1", "t2"), listOf("c1")))

        var links = linkService.get(TEST_USER, PageRequest(1, 1))
        ...
        links = linkService.get(TEST_USER, PageRequest(2, 1))
        assertThat(links.content).hasSize(1)
        ...
        links = linkService.get(TEST_USER, PageRequest(1, 3))
        assertThat(links.content).hasSize(3)
        assertThat(links.page).isEqualTo(1L)
        assertThat(links.size).isEqualTo(3)
        assertThat(links.total).isEqualTo(3)

        links = linkService.get(TEST_USER, PageRequest(1, 10))
        assertThat(links.content).hasSize(3)
        assertThat(links.page).isEqualTo(1L)
    ...
    fun testGetLinksSortOrdering() {
        ...
        val links = linkService.get(TEST_USER, PageRequest(1, 10, sort = "dateCreated", direction = SortDirection.ASC))
        assertThat(links.content).extracting("title").containsExactly("n1", "n2", "n3")

        val links2 = linkService.get(TEST_USER, PageRequest(1, 10, sort = "dateCreated", direction = SortDirection.DESC))
        assertThat(links2.content).extracting("title").containsExactly("n3", "n2", "n1")
    }

    @Test
    fun testGetLinksByGroup() {
        linkService.add(TEST_USER, newLink("l1", "google.com", listOf("t1", "t2"), listOf("c1")))
        linkService.add(TEST_USER, newLink("l2", "amazon.com", listOf("t1")))
        linkService.add(TEST_USER, newLink("l3", "netflix.com", emptyList(), listOf("c2")))
        linkService.add(TEST_USER, newLink("l4", "fb.com"))

        val onlyTags = linkService.get(TEST_USER, PageRequest(tags = listOf("t1")))
        assertThat(onlyTags.content).hasSize(2)
        assertThat(onlyTags.content).extracting("title").containsExactlyInAnyOrder("l1", "l2")

        val onlyTags2 = linkService.get(TEST_USER, PageRequest(tags = listOf("t2")))
        assertThat(onlyTags2.content).hasSize(1)
        assertThat(onlyTags2.content).extracting("title").containsExactlyInAnyOrder("l1")

        val onlyCollections = linkService.get(TEST_USER, PageRequest(collections = listOf("c1")))
        ...
        val onlyCollections2 = linkService.get(TEST_USER, PageRequest(collections = listOf("c2")))
        ...
        val both = linkService.get(TEST_USER, PageRequest(tags = listOf("t1"), collections = listOf("c1")))
    ...
    fun testGetLinksBySource() {
        val l1 = linkService.add(TEST_USER, newLink("l1", "google.com/page1", listOf("t1", "t2"), listOf("c1")))
        val l2 = linkService.add(TEST_USER, newLink("l2", "amazon.com/content/more", listOf("t1")))
        val linksFromSource = linkService.get(TEST_USER, PageRequest(source = "amazon.com"))
        assertThat(linksFromSource.total).isEqualTo(1)
        assertThat(linksFromSource.content).hasSize(1).extracting<EntryId> { it.id }
            .containsOnly(l2.id)
        val linksByWildcardSource = linkService.get(TEST_USER, PageRequest(source = "goog%"))
        ...
        assertThat(linksByWildcardSource.content).hasSize(1).extracting<EntryId> { it.id }
            .containsOnly(l1.id)
        val linksFromMissingSource = linkService.get(TEST_USER, PageRequest(source = "invalid"))
    ...
    @Test
    fun testDeleteTags() {
        val added1 = linkService.add(TEST_USER, newLink("n1", "google.com", listOf("t1")))
        val added2 = linkService.add(TEST_USER, newLink("n12", "gmail.com", listOf("t1", "t2")))

        assertThat(linkService.get(TEST_USER, added1.id)?.tags).hasSize(1).extracting("id").containsExactly("t1")
        assertThat(linkService.get(TEST_USER, added2.id)?.tags).hasSize(2).extracting("id").containsExactly("t1", "t2")

        tagService.delete(TEST_USER, "t2")

        assertThat(linkService.get(TEST_USER, added1.id)?.tags).hasSize(1).extracting("id").containsExactly("t1")
        assertThat(linkService.get(TEST_USER, added2.id)?.tags).hasSize(1).extracting("id").containsExactly("t1")

        tagService.delete(TEST_USER, "t1")

        assertThat(linkService.get(TEST_USER, added1.id)?.tags).isEmpty()
        assertThat(linkService.get(TEST_USER, added2.id)?.tags).isEmpty()
    }

    @Test
    fun testDeleteCollections() {
        val added1 = linkService.add(TEST_USER, newLink("n1", "google.com", emptyList(), listOf("c1")))
        val added2 = linkService.add(TEST_USER, newLink("n12", "gmail.com", emptyList(), listOf("c1", "c2")))

        assertThat(linkService.get(TEST_USER, added1.id)?.collections).hasSize(1).extracting("id").containsExactly("c1")
        assertThat(linkService.get(TEST_USER, added2.id)?.collections).hasSize(2).extracting("id").containsExactly("c1", "c2")

        collectionService.delete(TEST_USER, "c2")

        assertThat(linkService.get(TEST_USER, added1.id)?.collections).hasSize(1).extracting("id").containsExactly("c1")
        assertThat(linkService.get(TEST_USER, added2.id)?.collections).hasSize(1).extracting("id").containsExactly("c1")

        collectionService.delete(TEST_USER, "c1")

        assertThat(linkService.get(TEST_USER, added1.id)?.collections).isEmpty()
        assertThat(linkService.get(TEST_USER, added2.id)?.collections).isEmpty()
    }

    @Test
    fun testDeleteLink() {
        assertThat(linkService.delete(TEST_USER, EntryId("invalid"))).isFalse()

        val added1 = linkService.add(TEST_USER, newLink("n1", "google.com"))
        val added2 = linkService.add(TEST_USER, newLink("n12", "amazon.com"))

        assertThat(linkService.delete(TEST_USER, EntryId("e1"))).isFalse()
        assertThat(linkService.delete(TEST_USER, added1.id)).isTrue()

        assertThat(linkService.get(TEST_USER).content).hasSize(1)
        assertThat(linkService.get(TEST_USER, added1.id)).isNull()

        assertThat(linkService.delete(TEST_USER, added2.id)).isTrue()

        assertThat(linkService.get(TEST_USER).content).isEmpty()
        assertThat(linkService.get(TEST_USER, added2.id)).isNull()
    }

    @Test
    fun testUpdateExistingLink() {
        val added1 = linkService.add(TEST_USER, newLink("n1", "google.com"))
        assertThat(linkService.get(TEST_USER, added1.id)?.title).isEqualTo("n1")
        assertThat(linkService.get(TEST_USER, added1.id)?.tags).isEmpty()
        assertThat(linkService.get(TEST_USER, added1.id)?.collections).isEmpty()

        val updated = linkService.update(TEST_USER, newLink(added1.id, "updated", "amazon.com", listOf("t1"), listOf("c2")))
        val newLink = linkService.get(TEST_USER, updated!!.id)
        ...
        val oldLink = linkService.get(TEST_USER, added1.id)
        ...
        verify(exactly = 2) { workerRegistry.acceptDiscussionWork(TEST_USER, added1.id) }
    ...
    fun testUpdateLinkNoProcessFlag() {
        ...
        val updated = linkService.update(TEST_USER, newLink(added1.id, "updated", "amazon.com").copy(process = false))
        val newLink = linkService.get(TEST_USER, updated!!.id)
        assertThat(newLink?.id).isEqualTo(added1.id)
        assertThat(newLink?.title).isEqualTo("updated")
        assertThat(newLink?.url).isEqualTo("https://amazon.com")
        ...
        verify(exactly = 2) { workerRegistry.acceptLinkWork(any()) }
        verify(exactly = 1) { workerRegistry.acceptDiscussionWork(TEST_USER, added1.id) }
        verify(exactly = 2) { entryAuditService.acceptAuditEvent(added1.id, any(), any()) }
    }

    @Test
    fun testUpdateLinkTags() {
        val added1 = linkService.add(TEST_USER, newLink("n1", "google.com", listOf("t1", "t2")))
        assertThat(linkService.get(TEST_USER, added1.id)?.title).isEqualTo("n1")
        assertThat(linkService.get(TEST_USER, added1.id)?.url).isEqualTo("https://google.com")
        assertThat(linkService.get(TEST_USER, added1.id)?.tags).extracting("id").containsExactlyInAnyOrder("t1", "t2")

        linkService.update(TEST_USER, newLink(added1.id, "n1", "google.com", listOf("t2")))
        assertThat(linkService.get(TEST_USER, added1.id)?.title).isEqualTo("n1")
        assertThat(linkService.get(TEST_USER, added1.id)?.url).isEqualTo("https://google.com")
        assertThat(linkService.get(TEST_USER, added1.id)?.tags).extracting("id").containsExactlyInAnyOrder("t2")

        linkService.update(TEST_USER, newLink(added1.id, "n1", "google.com", listOf("t2", "t3")))
        assertThat(linkService.get(TEST_USER, added1.id)?.tags).extracting("id").containsExactlyInAnyOrder("t2", "t3")
        verify(exactly = 3) { entryAuditService.acceptAuditEvent(added1.id, any(), any()) }
    }

    @Test
    fun testUpdateLinkCollections() {
        val added1 = linkService.add(TEST_USER, newLink("n1", "google.com", emptyList(), listOf("c1", "c2")))
        assertThat(linkService.get(TEST_USER, added1.id)?.title).isEqualTo("n1")
        assertThat(linkService.get(TEST_USER, added1.id)?.url).isEqualTo("https://google.com")
        assertThat(linkService.get(TEST_USER, added1.id)?.collections).extracting("id").containsExactlyInAnyOrder("c1", "c2")

        linkService.update(TEST_USER, newLink(added1.id, "n1", "google.com", emptyList(), listOf("c2")))
        assertThat(linkService.get(TEST_USER, added1.id)?.title).isEqualTo("n1")
        assertThat(linkService.get(TEST_USER, added1.id)?.url).isEqualTo("https://google.com")
        assertThat(linkService.get(TEST_USER, added1.id)?.collections).extracting("id").containsExactlyInAnyOrder("c2")

        linkService.update(TEST_USER, newLink(added1.id, "n1", "google.com"))
        assertThat(linkService.get(TEST_USER, added1.id)?.collections).extracting("id").isEmpty()
        verify(exactly = 3) { entryAuditService.acceptAuditEvent(added1.id, any(), any()) }
    }
    ...
    fun testUpdateLinkNoId() {
        ...
        val updated = linkService.update(TEST_USER, newLink("updated", "amazon.com"))
        assertThat(linkService.get(TEST_USER, updated!!.id)?.id).isNotEqualTo(added1.id)
    ...
    fun testUpdatePropsAttributesDoesntUpdate() {
        val added = linkService.add(TEST_USER, newLink("n1", "google.com"))
        added.props.addAttribute("key1", "attribute1")
        added.props.addAttribute("key2", "attribute2")

        linkService.update(TEST_USER, added)

        val updated = linkService.get(TEST_USER, added.id)
        assertThat(updated?.dateUpdated).isEqualTo(updated?.dateCreated)
    ...
    fun testMergeProps() {
        ...
        val task = TaskDefinition(TaskId("t1"), "description", "className")
        added.props.addTask(task)
        linkService.mergeProps(TEST_USER, added.id, added.props)
        ...
        linkService.mergeProps(TEST_USER, added.id, updatedProps)
    ...
    fun testVersioning() {
        ...
        val version1 = linkService.get(TEST_USER, added.id, 1)
        ...
        val updated = linkService.update(TEST_USER, newLink(added.id, "edited", "fb.com"))
        val version2 = linkService.get(TEST_USER, added.id, 2)
        ...
        // get original
        val first = linkService.get(TEST_USER, added.id, 1)
        ...
        // update directly
        val updatedDirect =
            linkService.update(TEST_USER, updated!!.copy(title = "new title", thumbnailId = ResourceId("r1")), true)
        val version3 = linkService.get(TEST_USER, added.id)
        ...
        // get version before
        val stepBack = linkService.get(TEST_USER, added.id, 2)
        ...
        // get current version
        val current = linkService.get(TEST_USER, added.id)
    ...
    fun testGetInvalidVersion() {
        val added = linkService.add(TEST_USER, newLink("n1", "google.com"))
        assertThat(linkService.get(TEST_USER, added.id, 0)).isNull()
        assertThat(linkService.get(TEST_USER, added.id, 2)).isNull()
        assertThat(linkService.get(TEST_USER, added.id, -1)).isNull()
        assertThat(linkService.get(TEST_USER, EntryId("invalid"), 0)).isNull()
    }
    ...
    fun testSetRead() {
        ...
        val read = linkService.read(TEST_USER, added.id, true)
        ...
        val unread = linkService.read(TEST_USER, added.id, false)
        assertThat(unread?.read).isFalse()
        assertThat(unread?.version).isOne()
        assertThat(unread?.dateUpdated).isEqualTo(added.dateUpdated)

        assertThat(linkService.get(TEST_USER, added.id)?.read).isFalse()
    }

    @Test
    fun testSetReadInvalidLink() {
        assertThat(linkService.read(TEST_USER, EntryId("invalid"), true)).isNull()
        assertThat(linkService.read(TEST_USER, EntryId("invalid"), false)).isNull()
    }
    ...
    fun testGetUnreadLinks() {
        ...
        assertThat(linkService.getUnread(TEST_USER)).hasSize(1).extracting<EntryId> { it.id }
            .containsOnly(added.id)

        linkService.read(TEST_USER, added.id, false)
        assertThat(linkService.getUnread(TEST_USER)).hasSize(1).extracting<EntryId> { it.id }
            .containsOnly(added.id)

        linkService.read(TEST_USER, added.id, true)
        assertThat(linkService.getUnread(TEST_USER)).isEmpty()
    }

    @Test
    fun testGetDeadLinks() {
        val added = linkService.add(TEST_USER, newLink("n1", "google.com"))
        assertThat(linkService.getDead(TEST_USER)).isEmpty()

        added.props.addAttribute(DEAD_LINK_PROP, true)
        linkService.mergeProps(TEST_USER, added.id, added.props)
        assertThat(linkService.getDead(TEST_USER)).hasSize(1).extracting<EntryId> { it.id }
    ...
    fun testCheckExistingLinkWithUrl() {
        val added1 = linkService.add(TEST_USER, newLink("n1", "google.com/content?param=one"))
        linkService.add(TEST_USER, newLink("n1", "google.co.uk/content"))
        val added2 = linkService.add(TEST_USER, newLink("n1", "google.com/content?param=one"))

        val existing = linkService.checkExistingWithUrl(TEST_USER, "google.com/content?param=one")
        assertThat(existing).hasSize(2).extracting<EntryId> { it.id }
            .containsExactly(added1.id, added2.id)

        assertThat(linkService.checkExistingWithUrl(TEST_USER, "amazon.com")).isEmpty()
    }
    ...
    fun testUpdateSearchableContent() {
        ...
        val result = linkService.updateSearchableContent(TEST_USER, added.id, updatedContent)
    ...
    fun testUpdateSearchableContentNotFound() {
        val result = linkService.updateSearchableContent(TEST_USER, EntryId("invalid"), "updated content")
