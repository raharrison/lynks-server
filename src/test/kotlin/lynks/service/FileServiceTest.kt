...
class FileServiceTest : DatabaseTest() {
    ...
    fun testCreateBasicFile() {
        val file = fileService.add(TEST_USER, newFile(EntryId("f1"), "filename"))
    ...
    fun testCreateFileWithTags() {
        val file = fileService.add(TEST_USER, newFile(EntryId("f1"), "filename", listOf("t1", "t2")))
        assertThat(file.type).isEqualTo(EntryType.FILE)
        assertThat(file.title).isEqualTo("filename")
    ...
    fun testCreateFileWithInvalidTag() {
        assertThrows<InvalidModelException> {
            fileService.add(
                TEST_USER,
                newFile(EntryId("f1"), "filename", listOf("t1", "invalid"))
            )
        }
    }
    ...
    fun testCreateFileWithCollections() {
        val file = fileService.add(TEST_USER, newFile(EntryId("f1"), "filename", cols = listOf("c1", "c2")))
        assertThat(file.type).isEqualTo(EntryType.FILE)
        assertThat(file.title).isEqualTo("filename")
    ...
    @Test
    fun testGetFileById() {
        fileService.add(TEST_USER, newFile(EntryId("f1"), "filename", listOf("t1", "t2"), listOf("c1")))
        val file2 = fileService.add(TEST_USER, newFile(EntryId("f2"), "filename", listOf("t2"), listOf("c2")))
        val retrieved = fileService.get(TEST_USER, file2.id)
    ...
    fun testGetFileDoesntExist() {
        assertThat(fileService.get(TEST_USER, EntryId("invalid"))).isNull()
    }

    @Test
    fun testGetFilesPage() {
        fileService.add(TEST_USER, newFile(EntryId("f1"), "filename1", listOf("t1", "t2"), listOf("c1")))
        Thread.sleep(10)
        fileService.add(TEST_USER, newFile(EntryId("f2"), "filename2", listOf("t1", "t2"), listOf("c1")))
        Thread.sleep(10)
        fileService.add(TEST_USER, newFile(EntryId("f3"), "filename3", listOf("t1", "t2"), listOf("c1")))

        var files = fileService.get(TEST_USER, PageRequest(1, 1))
        ...
        files = fileService.get(TEST_USER, PageRequest(2, 1))
        assertThat(files.content).hasSize(1)
        ...
        files = fileService.get(TEST_USER, PageRequest(1, 3))
        assertThat(files.content).hasSize(3)
        assertThat(files.page).isEqualTo(1L)
        assertThat(files.size).isEqualTo(3)
        assertThat(files.total).isEqualTo(3)

        files = fileService.get(TEST_USER, PageRequest(1, 10))
        assertThat(files.content).hasSize(3)
        assertThat(files.page).isEqualTo(1L)
    ...
    fun testGetFilesSortOrdering() {
        ...
        val files = fileService.get(TEST_USER, PageRequest(1, 10, sort = "dateCreated", direction = SortDirection.ASC))
        assertThat(files.content).extracting("title").containsExactly("filename1", "filename2", "filename3")

        val files2 = fileService.get(TEST_USER, PageRequest(1, 10, sort = "dateCreated", direction = SortDirection.DESC))
    ...
    fun testGetFilesByGroup() {
        fileService.add(TEST_USER, newFile(EntryId("f1"), "filename1", listOf("t1", "t2"), listOf("c1")))
        fileService.add(TEST_USER, newFile(EntryId("f2"), "filename2", listOf("t1")))
        fileService.add(TEST_USER, newFile(EntryId("f3"), "filename3", emptyList(), listOf("c2")))
        fileService.add(TEST_USER, newFile(EntryId("f4"), "filename3"))

        val onlyTags = fileService.get(TEST_USER, PageRequest(tags = listOf("t1")))
        ...
        val onlyTags2 = fileService.get(TEST_USER, PageRequest(tags = listOf("t2")))
        assertThat(onlyTags2.content).hasSize(1)
        assertThat(onlyTags2.content).extracting("title").containsExactlyInAnyOrder("filename1")

        val onlyCollections = fileService.get(TEST_USER, PageRequest(collections = listOf("c1")))
        ...
        val onlyCollections2 = fileService.get(TEST_USER, PageRequest(collections = listOf("c2")))
        ...
        val both = fileService.get(TEST_USER, PageRequest(tags = listOf("t1"), collections = listOf("c1")))
    ...
    @Test
    fun testDeleteTags() {
        val added1 = fileService.add(TEST_USER, newFile(EntryId("f1"), "filename1", listOf("t1")))
        val added2 = fileService.add(TEST_USER, newFile(EntryId("f2"), "filename2", listOf("t1", "t2")))

        assertThat(fileService.get(TEST_USER, added1.id)?.tags).hasSize(1).extracting("id").containsExactly("t1")
        assertThat(fileService.get(TEST_USER, added2.id)?.tags).hasSize(2).extracting("id").containsExactly("t1", "t2")

        tagService.delete(TEST_USER, "t2")

        assertThat(fileService.get(TEST_USER, added1.id)?.tags).hasSize(1).extracting("id").containsExactly("t1")
        assertThat(fileService.get(TEST_USER, added2.id)?.tags).hasSize(1).extracting("id").containsExactly("t1")

        tagService.delete(TEST_USER, "t1")

        assertThat(fileService.get(TEST_USER, added1.id)?.tags).isEmpty()
        assertThat(fileService.get(TEST_USER, added2.id)?.tags).isEmpty()
    }

    @Test
    fun testDeleteCollections() {
        val added1 = fileService.add(TEST_USER, newFile(EntryId("f1"), "filename1", emptyList(), listOf("c1")))
        val added2 = fileService.add(TEST_USER, newFile(EntryId("f2"), "filename2", emptyList(), listOf("c1", "c2")))

        assertThat(fileService.get(TEST_USER, added1.id)?.collections).hasSize(1).extracting("id").containsExactly("c1")
        assertThat(fileService.get(TEST_USER, added2.id)?.collections).hasSize(2).extracting("id").containsExactly("c1", "c2")

        collectionService.delete(TEST_USER, "c2")

        assertThat(fileService.get(TEST_USER, added1.id)?.collections).hasSize(1).extracting("id").containsExactly("c1")
        assertThat(fileService.get(TEST_USER, added2.id)?.collections).hasSize(1).extracting("id").containsExactly("c1")

        collectionService.delete(TEST_USER, "c1")

        assertThat(fileService.get(TEST_USER, added1.id)?.collections).isEmpty()
        assertThat(fileService.get(TEST_USER, added2.id)?.collections).isEmpty()
    }

    @Test
    fun testDeleteFile() {
        assertThat(fileService.delete(TEST_USER, EntryId("invalid"))).isFalse()

        val added1 = fileService.add(TEST_USER, newFile(EntryId("f1"), "filename1"))
        val added2 = fileService.add(TEST_USER, newFile(EntryId("f2"), "filename2"))

        every { resourceManager.deleteAll(any()) } returns true

        assertThat(fileService.delete(TEST_USER, EntryId("e1"))).isFalse()
        assertThat(fileService.delete(TEST_USER, added1.id)).isTrue()

        assertThat(fileService.get(TEST_USER).content).hasSize(1)
        assertThat(fileService.get(TEST_USER, added1.id)).isNull()

        assertThat(fileService.delete(TEST_USER, added2.id)).isTrue()

        assertThat(fileService.get(TEST_USER).content).isEmpty()
        assertThat(fileService.get(TEST_USER, added2.id)).isNull()
    ...
    fun testUpdateExistingFile() {
        ...
        val updated = fileService.update(TEST_USER, newFile(added1.id, "new filename", listOf("t1"), listOf("c1")))
        val newFile = fileService.get(TEST_USER, updated!!.id)
        ...
        val oldFile = fileService.get(TEST_USER, added1.id)
    ...
    fun testUpdateFileTags() {
        val added1 = fileService.add(TEST_USER, newFile(EntryId("f1"), "filename 1", listOf("t1", "t2")))
        assertThat(fileService.get(TEST_USER, added1.id)?.title).isEqualTo("filename 1")
        assertThat(fileService.get(TEST_USER, added1.id)?.tags).extracting("id").containsExactlyInAnyOrder("t1", "t2")

        fileService.update(TEST_USER, newFile(added1.id, "filename 1", listOf("t2")))
        assertThat(fileService.get(TEST_USER, added1.id)?.title).isEqualTo("filename 1")
        assertThat(fileService.get(TEST_USER, added1.id)?.tags).extracting("id").containsExactlyInAnyOrder("t2")

        fileService.update(TEST_USER, newFile(added1.id, "filename 1", listOf("t2", "t3")))
        assertThat(fileService.get(TEST_USER, added1.id)?.tags).extracting("id").containsExactlyInAnyOrder("t2", "t3")
        verify(exactly = 3) { entryAuditService.acceptAuditEvent(added1.id, any(), any()) }
    }

    @Test
    fun testUpdateFileCollections() {
        val added1 = fileService.add(TEST_USER, newFile(EntryId("f1"), "filename 1", emptyList(), listOf("c1", "c2")))
        assertThat(fileService.get(TEST_USER, added1.id)?.title).isEqualTo("filename 1")
        assertThat(fileService.get(TEST_USER, added1.id)?.collections).extracting("id").containsExactlyInAnyOrder("c1", "c2")

        fileService.update(TEST_USER, newFile(added1.id, "filename 1", emptyList(), listOf("c2")))
        assertThat(fileService.get(TEST_USER, added1.id)?.title).isEqualTo("filename 1")
        assertThat(fileService.get(TEST_USER, added1.id)?.collections).extracting("id").containsExactlyInAnyOrder("c2")
        verify { entryAuditService.acceptAuditEvent(added1.id, any(), any()) }

        fileService.update(TEST_USER, newFile(added1.id, "filename 1", emptyList(), emptyList()))
        assertThat(fileService.get(TEST_USER, added1.id)?.collections).extracting("id").isEmpty()
    }

    @Test
    fun testUpdateFileNoId() {
        val added1 = fileService.add(TEST_USER, newFile(EntryId("f1"), "filename1"))
        assertThat(fileService.get(TEST_USER, added1.id)?.title).isEqualTo("filename1")

        val updated = fileService.update(TEST_USER, newFile(title = "new content"))
        assertThat(fileService.get(TEST_USER, updated!!.id)?.id).isNotEqualTo(added1.id)
    ...
    fun testUpdatePropsAttributes() {
        val added = fileService.add(TEST_USER, newFile(EntryId("f1"), "filename1"))
        added.props.addAttribute("key1", "attribute1")
        added.props.addAttribute("key2", "attribute2")

        fileService.update(TEST_USER, added)

        val updated = fileService.get(TEST_USER, added.id)
    ...
    fun testMergeProps() {
        ...
        fileService.mergeProps(TEST_USER, added.id, updatedProps)
    ...
    @Test
    fun testVersioning() {
        val added = fileService.add(TEST_USER, newFile(EntryId("f1"), "filename"))
        ...
        val version1 = fileService.get(TEST_USER, added.id, 1)
        ...
        val updated = fileService.update(TEST_USER, newFile(added.id, "filename2"))
        val version2 = fileService.get(TEST_USER, added.id, 2)
    ...
    fun testGetInvalidVersion() {
        val added = fileService.add(TEST_USER, newFile(EntryId("f1"), "filename"))
        assertThat(fileService.get(TEST_USER, added.id, 0)).isNull()
        assertThat(fileService.get(TEST_USER, added.id, 2)).isNull()
        assertThat(fileService.get(TEST_USER, added.id, -1)).isNull()
        assertThat(fileService.get(TEST_USER, EntryId("invalid"), 0)).isNull()
    }
