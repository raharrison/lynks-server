...
class ScheduleServiceTest : DatabaseTest() {
    ...
    fun testAddNewReminderDirectly() {
        val schedule = scheduleService.add(reminder("sid", "e1", ReminderType.ADHOC, 100))
    ...
    fun testAddNewReminder() {
        val reminder1 = NewReminder(null, "e1", ReminderType.ADHOC, "", "100", tz)
        val reminder2 = NewReminder(null, "e1", ReminderType.RECURRING, "", "every", tz)
    ...
    fun testAddReminderInvalidTimeZone() {
        assertThrows<IllegalArgumentException> {
            scheduleService.addReminder(NewReminder(null, "e1", ReminderType.ADHOC, "", "100", "invalid"))
        }
    }

    @Test
    fun testAddReminderGivenIdDoesNotPerformUpdate() {
        val rem = scheduleService.addReminder(NewReminder(null, "e1", ReminderType.ADHOC, "", "100", tz))
        val res = scheduleService.addReminder(NewReminder(rem.reminderId, "e1", ReminderType.RECURRING, "", "200", tz))
    ...
    fun testAddScheduleNoEntry() {
        assertThrows<SQLException> {
            scheduleService.add(reminder("sid", "nothing", ReminderType.ADHOC, 100))
        }
        assertThrows<SQLException> {
            scheduleService.addReminder(NewReminder(null, "nothing", ReminderType.ADHOC, "", "100", tz))
        }
    }

    @Test
    fun testGetAllReminders() {
        scheduleService.add(reminder("sid", "e1", ReminderType.ADHOC, 100))
        scheduleService.add(reminder("sid2", "e1", ReminderType.RECURRING, 200))
        scheduleService.add(reminder("sid3", "e2", ReminderType.RECURRING, 300))
    ...
    fun testGetSchedulesReturnsType() {
        val s1 = scheduleService.add(reminder("sid", "e1", ReminderType.ADHOC, 100))
        val s2 = scheduleService.add(reminder("sid3", "e2", ReminderType.RECURRING, 300))
    ...
    fun testUpdateReminderNoRow() {
        assertThat(scheduleService.updateReminder(NewReminder("invalid", "e1", ReminderType.ADHOC, "", "300", tz))).isNull()
    }

    @Test
    fun testUpdateReminderNoId() {
        val res = scheduleService.updateReminder(NewReminder(null, "e1", ReminderType.ADHOC, "", "300", tz))
    ...
    fun testUpdateReminder() {
        val res1 = scheduleService.addReminder(NewReminder(null, "e1", ReminderType.ADHOC, "", "100", tz))
        val res2 = scheduleService.addReminder(NewReminder(null, "e1", ReminderType.RECURRING, "", "200", tz))
        assertThat(scheduleService.getAllReminders()).hasSize(2).extracting("reminderId").doesNotHaveDuplicates()

        val updated = scheduleService.updateReminder(NewReminder(res1.reminderId, "e1", ReminderType.RECURRING, "", "500", tz))
        ...
        val updated2 = scheduleService.updateReminder(NewReminder(res2.reminderId, "e2", ReminderType.ADHOC, "", "800", "America/New_York"))
    ...
    fun testUpdateReminderInvalidTimeZone() {
        val res1 = scheduleService.addReminder(NewReminder(null, "e1", ReminderType.ADHOC, "", "100", tz))
        assertThrows<IllegalArgumentException> {
            scheduleService.updateReminder(NewReminder(res1.reminderId, "e1", ReminderType.RECURRING, "", "500", "invalid"))
        }
    }

    @Test
    fun testDeleteSchedule() {
        val s1 = scheduleService.add(reminder("sid", "e2", ReminderType.RECURRING, 300))
        assertThat(scheduleService.getAllReminders()).hasSize(1)
    ...
    private fun reminder(sid: String, eId: String, type: ReminderType, interval: Long = 0): Reminder {
        return when (type) {
            ReminderType.ADHOC -> AdhocReminder(sid, entryId = eId, message = null, interval = interval, tz = this.tz)
            ReminderType.RECURRING -> RecurringReminder(sid, entryId = eId, message = null, fire = interval.toString(), tz = this.tz)
        }
    }
